...
const plugin = {
  ...
  async _persistTableData(app, sectionName, userDayRatingResponse) {
    ...
    tableMarkdown += `| [${ this._bulletNoteHandle.name }](/notes/${ this._bulletNoteHandle.uuid }) | ${ receivedDayRating ? userDayRatingResponse[0] : "See note" } | ${ receivedDayRating ? userDayRatingResponse[1] : "See note" } | ${ (new Date()).toLocaleDateString() } |\n`;
