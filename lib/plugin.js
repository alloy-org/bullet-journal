  ...
  async _persistTableData(app, sectionName, userDayRatingResponse) {
    ...
    if (receivedDayRating) {
      ...
      if (existingJournalContent?.includes("# Day Rating")) {
        ...
        await app.replaceNoteContent(this._bulletNoteHandle,
          `${ sectionContent }\n- Rating given at ${ (new Date()).toLocaleTimeString(navigator.language, { second: false }) }: ${ userDayRatingResponse[0] }${ userDayRatingResponse[1]?.length ? `\n    - Precipitating factors: ${ userDayRatingResponse[1] }` : "" }`,
          { heading: { text: "Day Rating" }});

      } else {
        await app.insertNoteContent(this._bulletNoteHandle,
          `# Day Rating\n* Rating given at ${ (new Date()).toLocaleTimeString(navigator.language, { second: false }) }: ${ userDayRatingResponse[0] || "N/A" }${ userDayRatingResponse[1]?.length ? `\n    - Precipitating factors: ${ userDayRatingResponse[1] }` : "" }`,
          { atEnd: true }
        );
      }
    }
