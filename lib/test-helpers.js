...
// --------------------------------------------------------------------------------------
// Phind-generated, no one-approved
...
export const mockApp = seedNote => {
  const app = {};
  app.notes = {}; // key=uuid, value=noteHandle
  ...
  app.createNote = jest.fn().mockImplementation(async (content, tagArray) => {
    const uuid = generateUUID();
    app.notes[uuid] = { tags: tagArray, body: content };
    return uuid;
  });
  app.findNote = jest.fn().mockImplementation(async (noteHandle) => {
    if (noteHandle.uuid) {
      const note = app.notes[noteHandle.uuid];
      if (note) return note;
    }
    if (noteHandle.name) {
      return Object.values(app.notes).find(n => (n.name === noteHandle.name));
    }
  });
  app.getNoteContent = jest.fn().mockImplementation(async (noteHandle) => {
    return app.notes[noteHandle?.uuid]?.body;
  });
  app.insertNoteContent = jest.fn().mockImplementation(async (noteHandle, content, { atEnd = false } = {}) => {
    let note = app.notes[noteHandle?.uuid] || seedNote;
    if (!note) {
      console.error("Couldn't find a note from", noteHandle);
      throw("Couldn't find a note from noteHandle");
    }
    if (atEnd) {
      note.body += content;
    } else {
      note.body = `${ content }${ note.body }`;
    }
  })
